...
type Product = { id: string; productId: string; name: string; sku: string; barcode: string; category: string; categoryId: string | null; categoryParentId: string | null; unit: string; price: number; stock: number; minStock: number; warehouse: string; warehouseId: string; status: string; type?: string; createdAt?: string }
...
function ProductDrawer({ product, onClose }: { product: Product; onClose: () => void }) {
  const details = [
    ['Sana', formatTxDate(product.createdAt)],
    ['Kategoriya', product.category],
    ['Ombor', product.warehouse],
    ['Minimal qoldiq', `${product.minStock} ${product.unit}`],
    ['Holat', product.status],
  ]
  return <>
    <div className="fixed inset-0 z-40 bg-black/30" onClick={onClose} />
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md overflow-y-auto bg-card p-6 shadow-2xl">
      <div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary">Mahsulot tafsilotlari</p><h2 className="mt-2 text-xl font-bold">{product.name}</h2><p className="mt-1 font-mono text-xs text-muted-foreground">ID: {product.productId} • SKU: {product.sku}</p></div><Button variant="ghost" onClick={onClose}><X className="size-5" /></Button></div>
      <div className="mt-8 grid grid-cols-2 gap-3"><Card className="bg-muted/30 p-4"><div className="text-xs text-muted-foreground">Qoldiq</div><div className="mt-2 text-xl font-bold">{product.stock}</div><div className="text-xs text-muted-foreground">{product.unit}</div></Card><Card className="bg-muted/30 p-4"><div className="text-xs text-muted-foreground">Sotuv narxi</div><div className="mt-2 text-lg font-bold">{money(product.price)}</div></Card></div>
      {product.barcode && <Card className="mt-4 flex flex-col items-center bg-muted/20 p-4"><div className="mb-2 text-xs text-muted-foreground">Shtrix kod (ID: {product.productId})</div><BarcodeImage value={product.barcode} className="max-w-full" /><Button variant="outline" className="mt-3" onClick={() => downloadBarcodeLabel(product)}><Download className="size-4" />Shtrix kod + ID yuklab olish</Button></Card>}
      <div className="mt-8 flex flex-col gap-4">{details.map(([label, value]) => <div key={label} className="flex items-center justify-between border-b pb-3 text-sm"><span className="text-muted-foreground">{label}</span><span className="font-semibold">{value}</span></div>)}</div>
      <Button className="mt-8 w-full" onClick={onClose}>Yopish</Button>
    </div>
  </>
}
